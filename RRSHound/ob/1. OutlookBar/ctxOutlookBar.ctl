...
Private Const DEF_ORIENTATION       As Long = ucsportrait
...
Private m_lItemWidth           As Long
Private m_lGroupWidth           As Long
...
Private m_eOrientation          As UcsOrientation
...
Private Enum UcsScrollArrowEnum
    ...
    ucsScrollArrowRight = &H3
    ucsScrollArrowLeft = &H4
End Enum
...
Property Get Orientation() As UcsOrientation
    Orientation = m_eOrientation
End Property

Property Let Orientation(ByVal o As UcsOrientation)
    m_eOrientation = o
    pvPropertyChanged
End Property
...
Attribute HitTest.VB_HelpID = 127
    ...
    If m_eOrientation = ucsportrait Then
        ...
        If lBottom > lTop Then
            If lIdx >= 1 And lIdx <= Groups.Count Then
                ...
                If x >= INNER_BORDER And x < lWidth - INNER_BORDER And _
                    ...
                    pvGetItemInfo Groups(lIdx), lItemWidth, lItemHeight, 0, 0
    ...
    Else 'if m_eorientation = ucsLandscape then
    
        lWidth = ScaleHeight \ Screen.TwipsPerPixelX
        lGroupWidth = m_lGroupWidth
        lLeft = 0
        lRight = ScaleWidth \ Screen.TwipsPerPixelY
        
        For lIdx = 1 To Groups.Count
            If Groups(lIdx).Visible Then
                If y >= 1 And x >= lLeft And y <= lWidth - 1 And x <= lLeft + lGroupWidth Then
                    Set oBtn = Groups(lIdx)
                    HitTest = ucsHitGroupButton
                    Exit Function
                End If
                lLeft = lLeft + lGroupWidth + 1
            End If
            If Groups(lIdx).Selected Then
                Exit For
            End If
        ...
        For lIdx = Groups.Count To lIdx Step -1
            ...
            End If
            If Groups(lIdx).Visible Then
                lRight = lRight - lGroupWidth - 1
                If y >= 1 And x >= lRight And y < lWidth - 1 And x <= lRight + lGroupWidth Then
                    Set oBtn = Groups(lIdx)
                    HitTest = ucsHitGroupButton
                    Exit Function
                End If
        ...
        If lRight > lLeft Then
            If lIdx >= 1 And lIdx <= Groups.Count Then
                ...
                If y >= INNER_BORDER And y < lWidth - INNER_BORDER And _
                        x >= lLeft And x < lRight Then
                    '--- if inside -> the pt is at least on the group background
                    HitTest = ucsHitGroupBackground
                    pvGetItemInfo Groups(lIdx), lItemWidth, lItemHeight, 0, 0
                    Set oGroup = Groups(lIdx).GroupItems
                    '--- if scroll arrows are potentailly visible and near the right edge
                    If lLeft + (lItemWidth + ITEM_VERT_DISTANCE) * oGroup.Count > lRight _
                            And y <= lTop + INNER_BORDER + m_lScrollArrowSize Then
                        '--- if upper scroll arrow visible and over it
                        If m_lGroupOffset > 0 And _
                                x < lLeft + m_lScrollArrowSize Then
                            Set oBtn = Nothing
                            HitTest = ucsHitScrollupArrow
                            Exit Function
                        End If
                        '--- if lower scroll arrow visible and over it
                        If oGroup.Count * (lItemWidth + ITEM_VERT_DISTANCE) - m_lGroupOffset + 1 >= lRight - lLeft And _
                                x >= lRight - m_lScrollArrowSize Then
                            Set oBtn = Nothing
                            HitTest = ucsHitScrolldownArrow
                            Exit Function
                        End If
                    ...
                    For lIdx = 1 To oGroup.Count
                        If oGroup(lIdx).Visible Then
                            If x + m_lGroupOffset >= lLeft And x + m_lGroupOffset < lLeft + lItemWidth Then
                                Set oBtn = oGroup(lIdx)
                                HitTest = ucsHitItemButton
                                Exit Function
                            End If
                            lLeft = lLeft + lItemWidth + ITEM_VERT_DISTANCE
                        End If
                    Next
                End If
            End If
...
Attribute EnsureVisible.VB_HelpID = 106
    ...
    Dim lGroupVisibleWidth As Long
    Dim lOffset         As Long
    ...
    pvGetItemInfo oBtn.Parent, lItemWidth, lItemHeight, 0, 0
    ...
    lGroupVisibleWidth = ScaleWidth \ Screen.TwipsPerPixelX - lGroupVisibleCount * (m_lGroupWidth + 1) - 2 * (INNER_BORDER + m_oRendContrl.BorderSize) + 1
    '--- ensure enough offset
    
    If m_eOrientation = ucsportrait Then
        lOffset = pvMax(lBtnVisibleIdx * lItemHeight - lGroupVisibleHeight + 1, 0)
    Else
        lOffset = pvMax(lBtnVisibleIdx * lItemWidth - lGroupVisibleWidth + 1, 0)
    End If
    ...
    Else
        lOffset = (lBtnVisibleIdx - 1) * (lItemWidth + ITEM_VERT_DISTANCE)
    End If
...
'--- fix current group offset
    ...
    If m_lGroupOffset < 0 Then
        m_lGroupOffset = 0
    Else
        pvGetItemInfo m_oSelectedGroup, lItemWidth, lItemHeight, 0, 0
        If m_eOrientation = ucsportrait Then
        ...
        Else
            lMaxOffset = Groups.Count * (m_lGroupWidth + 1) + m_oSelectedGroup.GroupItems.Count * (lItemWidth + ITEM_VERT_DISTANCE) - (ScaleWidth \ Screen.TwipsPerPixelX) + 2 * INNER_BORDER + 2 * m_oRendContrl.BorderSize
        End If
...
Private Sub pvPaintGroup( _
    ...
    With oMemDC
        ...
        Else
            Select Case oFmt.HorAlignment
            Case ucsFhaRight
                lIconX = BottomY - 1 - lIconW
                'BottomY = BottomY - lIconW - 2
            Case Else
                lIconX = TopY + 1
                'TopY = TopY + lIconW + 2
            End Select
        ...
        Else
            ...
            Set tempDC = New cMemDC
            With tempDC
            
                .Init BottomY - TopY, RightX - LeftX, 0, 0
                Set .Font = oFmt.Font
            
                .BackStyle = BS_TRANSPARENT
        
                pvFillGradient tempDC, 0, 0, .Width, .Height, oFmt.BackGradient
            
                If Not oIcon Is Nothing Then
                    lIconY = (.Width - pvHM2Pix(oIcon.Height)) \ 2
                    If oBtn.Enabled Then
                        .PaintPicture oIcon, lIconX + oFmt.OffsetX, _
                            lIconY + oFmt.OffsetY, clrMask:=MaskColor
                    Else
                ...
                If oBtn.Enabled Then
                    .ForeColor = oFmt.ForeColor
                    .DrawText oBtn.Caption, oFmt.Padding + oFmt.OffsetX, oFmt.Padding + oFmt.OffsetY, .Width, tempDC.Height, lAlign
                Else
                    .ForeColor = vb3DHighlight
                    .DrawText oBtn.Caption, oFmt.Padding + oFmt.OffsetX + 1, oFmt.Padding + oFmt.OffsetY + 1, .Width + 1, .Height + 1, lAlign
                    .ForeColor = vbButtonShadow
                    .DrawText oBtn.Caption, oFmt.Padding + oFmt.OffsetX, oFmt.Padding + oFmt.OffsetY, .Width, .Height, lAlign
                End If
            
                'Rotate anti clockwise 90 degrees
                'For     Y = .Height - (oFmt.Padding + oFmt.OffsetY) To oFmt.Padding + oFmt.OffsetY Step -1
                '    For X = .Width- (oFmt.Padding + oFmt.OffsetX) To oFmt.Padding + oFmt.OffsetX Step -1
                '        omemdc.SetPixel LeftX + Y, TopY + X, .GetPixel(.Width - X, Y)
                '    Next X
                'Next Y
            
                'Rotate clockwise 90 degrees
                For y = .Height - (oFmt.Padding + oFmt.OffsetY) To oFmt.Padding + oFmt.OffsetY Step -1
                    For x = tempDC.Width - (oFmt.Padding + oFmt.OffsetX) To oFmt.Padding + oFmt.OffsetX Step -1
                        oMemDC.SetPixel LeftX + y, TopY + x, .GetPixel(x, .Height - y)
                    Next x
                Next y
...
Private Sub pvPaintArrow( _
    ...
    If Not m_bFlatScrollArrows Then
        ...
        oMemDC.DrawFrameControl LeftX, TopY, RightX, BottomY, DFC_SCROLL, _
                    IIf(lArrowType = ucsScrollArrowDown, DFCS_SCROLLDOWN, _
                        IIf(lArrowType = ucsScrollArrowUP, DFCS_SCROLLUP, _
                        IIf(lArrowType = ucsScrollArrowRight, DFCS_SCROLLRIGHT, _
                        IIf(lArrowType = ucsScrollArrowLeft, DFCS_SCROLLLEFT, 0))))
        ...
        With New cMemDC
            ...
            .DrawFrameControl -1, -1, .Width + 1, .Height + 1, DFC_SCROLL, _
                        IIf(lArrowType = ucsScrollArrowDown, DFCS_SCROLLDOWN, _
                            IIf(lArrowType = ucsScrollArrowUP, DFCS_SCROLLUP, _
                            IIf(lArrowType = ucsScrollArrowRight, DFCS_SCROLLRIGHT, _
                            IIf(lArrowType = ucsScrollArrowLeft, DFCS_SCROLLLEFT, 0)))) Or DFCS_FLAT ' Or IIf((m_lScrollBtnState And lArrowType) <> 0, DFCS_PUSHED, 0)
...
Private Sub pvPaintGroupItems( _
    ...
    Dim lItmLeft         As Long
    
    On Error GoTo EH
    pvGetItemInfo oGroup, lItemWidth, lItemHeight, lIconW, lIconH
    ...
    With oMemDC
            ...
            If lTop < lBottom Then
                ...
                If bShowDropHilight Then
                    If DropHighlightIdx >= 0 And DropHighlightIdx <= oGroup.GroupItems.Count Then
                        lY = lTop - lOffset + lItemHeight * DropHighlightIdx
        ...
        Else 'If m_eOrientation = ucsLandscape Then
            ...
            If lLeft < lRight Then
                .SetClipRect lLeft, lTop, lRight, lBottom
                lItmLeft = lLeft - lOffset
                For Each oBtn In oGroup.GroupItems
                    If oBtn.Visible Then
                        If lItmLeft + lItemWidth > lLeft Then
                            pvPaintItem oMemDC, lItmLeft, lTop, lItmLeft + lItemWidth, lBottom, oBtn, _
                                lIconW, lIconH, IIf(oGroup.IconsType = ucsIcsSmallIcons, oBtn.SmallIcon, oBtn.LargeIcon)
                                
                        End If
                        lItmLeft = lItmLeft + lItemWidth + ITEM_VERT_DISTANCE
                        '--- beyond bottom of clipping area
                        If lItmLeft > lRight Then
                            Exit For
                        End If
                    End If
                Next
                If bShowDropHilight Then
                    If DropHighlightIdx >= 0 And DropHighlightIdx <= oGroup.GroupItems.Count Then
                        lY = lLeft - lOffset + lItemWidth * DropHighlightIdx
                        pvPaintDropHilight oMemDC, lY, lTop, lRight, vbBlack
                    End If
                End If
                If bShowArrows Then
                    '--- right arrow
                    If lItmLeft >= lRight Then
                        pvPaintArrow oMemDC, lRight - m_lScrollArrowSize, lTop, _
                            lRight, lTop + m_lScrollArrowSize, ucsScrollArrowRight, oGroup.IconsType
                    End If
                    '--- left arrow
                    If lOffset > 0 Then
                        pvPaintArrow oMemDC, lLeft, lTop, lLeft + m_lScrollArrowSize, _
                            lTop + m_lScrollArrowSize, ucsScrollArrowLeft, oGroup.IconsType
                    End If
                End If
            End If
        End If
...
Private Sub pvPaintControl( _
    ...
    For Each oBtn In Groups
        ...
        If Not oIcon Is Nothing Then
            ...
            lIdx = pvHM2Pix(oIcon.Width) + 2 * lGroupBorder
            If m_lGroupWidth < lIdx Then
                m_lGroupWidth = lIdx
            End If
    ...
    With oMemDC
        ...
        Else 'If m_eOrientation = ucsPortrait Then
            ...
            For lIdx = Groups.Count To 2 Step -1
                ...
                If Groups(lIdx).Visible Then
                    lRight = lRight - lGroupWidth - 1
                    pvFillGradient oMemDC, lRight, lTop, lRight + 1, lBottom, FormatControl.BackGradient
                    pvPaintGroup oMemDC, lRight + 1, lTop, lRight + lGroupWidth + 1, lBottom, Groups(lIdx)
                End If
            Next
            lBtmIdx = lIdx
            '--- loop groups on top
            ...
            For lIdx = 1 To lBtmIdx
                If Groups(lIdx).Visible Then
                    lVisibleAtLeft = lVisibleAtLeft + 1
                    pvFillGradient oMemDC, lLeft, lTop, lLeft + 1, lBottom, FormatControl.BackGradient
                    pvPaintGroup oMemDC, lLeft + 1, lTop, lLeft + lGroupWidth + 1, lBottom, Groups(lIdx)
                    lLeft = lLeft + lGroupWidth + 1
                End If
                If Groups(lIdx).Selected Or Groups(lIdx) Is oPrevSlected Then
                    Exit For
            ...
            If lIdx < lBtmIdx Then
                For lMiddle = lIdx + 1 To lBtmIdx
                    lVisibleInBetween = lVisibleInBetween + Abs(Groups(lMiddle).Visible)
                Next
                lMiddle = lLeft + ((lRight - lLeft - lVisibleInBetween * (lGroupWidth + 1)) * lStep) \ (AnimationSteps + 1)
            Else
                lMiddle = lRight
            End If
            '--- paint upper visible group items
            If lRight > lLeft Then
                If lIdx > 0 And lIdx <= Groups.Count Then
                    pvPaintGroupItems oMemDC, lLeft, lTop, lMiddle, lBottom, Groups(lIdx), lRight - lMiddle - lVisibleInBetween * (lGroupWidth + 1) + m_lGroupOffset, lStep = 0, lIdx >= lBtmIdx
                End If
            End If
            If lIdx < lBtmIdx Then
                lLeft = lMiddle
            ...
            ElseIf GroupHighlightIdx >= 0 Then
                .SetClipRect -1
                '--- paint group hilight
                lLeft = (m_lGroupWidth + 1) * GroupHighlightIdx
                If GroupHighlightIdx >= lVisibleAtTop Then
                    lLeft = lLeft + lRight - (m_lGroupWidth + 1) * lVisibleAtTop
                End If
                pvPaintDropHilight oMemDC, lLeft, lTop, lRight, vbBlack
            End If
...
Private Sub pvGetItemInfo( _
            ByVal oGroup As cButton, _
            lItemWidth As Long, lItemHeight As Long, _
    ...
    With New cMemDC
        ...
        Else
            ...
            Set .Font = m_oRendItmNrm.Font
            '--- calc item width
            lItemHeight = .Height - 2 * (1 + INNER_BORDER + lItemBorder + lIconBorder) - 2
            If oIcnFmt.VertAlignment <> m_oRendItmNrm.VertAlignment And oIcnFmt.HorAlignment = m_oRendItmNrm.HorAlignment Or oIcnFmt.HorAlignment = ucsFhaCenter Then
            Else
                lItemHeight = lItemHeight - lIconHeight
            End If
            For Each oBtn In oGroup.GroupItems
                If InStr(oBtn.Caption, vbCrLf) > 0 Or WrapText Then
                    '--- calc number of lines
    ...
    Else
        If oIcnFmt.VertAlignment <> m_oRendItmNrm.VertAlignment And oIcnFmt.HorAlignment = m_oRendItmNrm.HorAlignment Or oIcnFmt.HorAlignment = ucsFhaCenter Then
            lItemWidth = lIconWidth + 2 * lIconBorder + lTextLines * m_lItemWidth + 2 * lItemBorder
        Else
            lItemWidth = pvMax(lIconWidth + 2 * lIconBorder, lTextLines * m_lItemWidth) + 2 * lItemBorder
        End If
    End If
...
'        Set .Font = FormatGroup.Font
        ...
        m_lItemWidth = .TextWidth("A") '+ 6 + 2 * pvLngUndef(m_oFmtContrl.Padding)
        m_lGroupWidth = .TextWidth("AB") + 4 + 2 * FormatGroup.Render.Padding
            ...
            If Not oGrp Is Nothing Then
                ...
                If m_oSelectedGroup.GroupItems.Count > 0 Then
                    ...
                    If Not oBtn Is Nothing Then
                        ...
                        Else
                            m_lGroupOffset = oBtn.Parent.GroupItems.Count * (lItemWidth + ITEM_VERT_DISTANCE)
                        End If
    ...
    If (Button And vbLeftButton) <> 0 Then
        ...
        If m_oPressed.Enabled Then
            ...
            Case ucsHitScrollupArrow, ucsHitScrolldownArrow
                ...
                Else
                    pvAnimateItems m_lGroupOffset + IIf(eHit = ucsHitScrollupArrow, -ScrollItemsCount, ScrollItemsCount) * (lItemWidth + ITEM_VERT_DISTANCE)
                End If
                m_lScrollBtnState = 0
            Case Else
                RefreshControl
    ...
    If Not m_oOleDragged Is Nothing Then
        ...
        If m_oOleDragged.Class = ucsBtnClassItem Then
            ...
            Case ucsHitScrollupArrow
                ...
                Else
                    pvAnimateItems m_lGroupOffset - ScrollItemsCount * (lItemWidth + ITEM_VERT_DISTANCE)
                End If
                RefreshControl
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Orientation = .ReadProperty("Orientation", DEF_OLEDROPMODE)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        Call .WriteProperty("Orientation", Orientation, DEF_ORIENTATION)
...
Private Sub m_oLabelEdit_Change()
    ...
    Case ucsBtnClassItem
        '--- get item position
        pvGetItemInfo m_oLabelItem.Parent, lItemWidth, lItemHeight, lIconW, lIconH
        ...
        Else
            TopY = m_oRendContrl.BorderSize + INNER_BORDER '+ (m_oLabelItem.Parent.Position * (m_lGroupHeight + 1) + (m_oLabelItem.Position - 1) * (lItemHeight + ITEM_VERT_DISTANCE) - m_lGroupOffset) - 1
            BottomY = ScaleHeight / Screen.TwipsPerPixelY - TopY
            LeftX = m_oRendContrl.BorderSize + INNER_BORDER + (m_oLabelItem.Parent.Position * (m_lGroupWidth + 1) + (m_oLabelItem.Position - 1) * (lItemWidth + ITEM_VERT_DISTANCE) - m_lGroupOffset) - 1
            RightX = LeftX + lItemWidth 'ScaleWidth / Screen.TwipsPerPixelX - LeftX
        End If
...
Private Sub ISubclassingSink_Before(bHandled As Boolean, lReturn As Long, hwnd As Long, uMsg As Long, wParam As Long, lParam As Long)
    ...
    Case MouseWheelFwdMsg ' WM_MOUSEWHEEL
        ...
        Case ucsHitGroupBackground, ucsHitItemButton
            ...
            Else
                pvAnimateItems m_lGroupOffset + IIf((wParam \ &H1000&) > 0, -ScrollItemsCount, ScrollItemsCount) * (lItemWidth + ITEM_VERT_DISTANCE)
            End If
